...
def add_variables(prob, num_periods, p_max, s_n):
    # 向问题中添加充电/放电功率变量 P_CS_t 和电池存储状态变量 S_t
...
def solve_problem(file_path, s_n, alpha, delta_t, output_file):
    # 载入负荷功率数据并设置问题参数
    ...
    # 创建一个 Matplotlib Figure，并添加两个子图
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 12))
    
    # 在第一个子图上绘制 P_L、P_CS 和 P_CS-P_L 的折线图
    ax1.plot(p_l, label='P_L')
    ax1.plot(solution_df['P_CS'], label='P_CS')
    ax1.plot(solution_df['P_CS'] + p_l, label='P_CS + P_L')
    # ax1.axhline(y=p_f_max, color='r', linestyle='--', label='y=4500')
    ...
    # 在第二个子图上绘制 S_t 的折线图
    ax2.plot(solution_df['S'], label='S')
    ax2.set_xlabel('Time Period')
    ax2.set_ylabel('Storage State')
    ax2.set_title('Storage State Profile')
    ax2.legend()
    ax2.grid(True)
    ...
    plt.tight_layout()
    # 保存图片
    plt.savefig("power_and_storage_profiles.png")
    
    # 显示图片
    plt.show()

if __name__ == "__main__":
    solve_problem("C:\\Codes\\Projects\\Demand_management_verify\\Data\\xr_12_filtered.csv", s_n=1800, alpha=0.85, delta_t=0.25, output_file="C:\\Codes\\Projects\\Demand_management_verify\\Data\\daily_opt.csv")
